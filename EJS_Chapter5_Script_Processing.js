...
// Higher order functions are exellent for data processing
...
// get an array of objects thar has the direction of rtl
let rtlScripts = SCRIPTS.filter(s => s.direction == "rtl");

// updatea the array using map method, return an array of language names
...
// using reduce method to get the sum of an array by updating the  current value
// in standard JS reduce method, if the array is not empty, we can take start away
...
// use JS reduce method, not the reduce method from above to find the script with the most characters
function characterCount(script){
    return script.ranges.reduce((count, [from, to]) => {return count + (to - from);}, 0);
}

// getting the language with most numbers of unicode
console.log(SCRIPTS.reduce((a, b) => {
  return characterCount(a) < characterCount(b) ? b : a;
}));

// or we still can workout the result using a more conventional method
let biggest = null;
...
// using higher-order functions will help us when we trying to composing stuff
function average(array) {
  return array.reduce((a, b) => a + b) / array.length;
}
// get the average invented year of living languanges and dead languages
console.log(Math.round(average(SCRIPTS.filter(s => s.living).map(s => s.year ))));
console.log(Math.round(average(SCRIPTS.filter(s => !s.livng).map(s => s.year))));

// test some condition with some method of JS
...
// count the chars belonging to each script
function countBy(items, groupName){
  let counts = [];
  for (let item of items){
    // test the condition passed as groupName, item > 2, name is true, item <= 2, name is false
    let name = groupName(item);
    // find the index of object that object.name is true or false
    let known = counts.findIndex(c => c.name == name);
    // if name not in counts, add the relevant object to counts
    if (known == -1) {
      counts.push({name, count: 1 });
    }
    else {
      // name in counts, increment the count
      counts[known].count++;
    }
  }
  return counts;
...
// find which scripts that are used in a piece of text
function textScripts(text){
  let scripts = countBy(text, char => {
    ...
    // map the result to a new array, then out put the joined result
    return scripts.map(({name, count}) => {
      return `${Math.round(count * 100/total)}% ${name}`;
    }).join(", ");
}

// output 61% Han, 22% Latin, 17% Cyrilli
